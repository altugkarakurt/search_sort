...
index of the largest entry less than $item instead of None. The return
value -1 means it is smaller than all terms in the list.
--------------------------------------------------------------------"""
def binary_search(item,lst,_max_lt_idx=False):
    ...
    while(left_idx <= right_idx):
        ...
        if(lst[mid_idx] < item):
            ...
            return mid_idx
    return None if(not _max_lt_idx) else right_idx
